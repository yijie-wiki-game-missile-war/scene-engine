...
def test_wire_v2_checkpoint_contains_world_and_display_baseline_only() -> None:
    ...
    assert raw[4] == WIRE_MAJOR_VERSION == 2
    assert packet.kind is PacketKind.CHECKPOINT
    assert packet.header["schema"] == WIRE_SCHEMA == "scene-engine-wire@2"
    assert packet.header["display_codec"] == DISPLAY_CODEC
...
def test_frozen_wire_v2_golden_packets_round_trip_exact_bytes() -> None:
    expected = {
    ...
    for name, kind in expected.items():
        raw = (FIXTURES / "wire-v2" / name).read_bytes()
        packet = read_engine_packet(raw)
        assert packet.kind is kind
        assert packet.raw_bytes == raw


def test_frozen_display_v2_corpus_validates_and_malformed_records_fail() -> None:
    root = FIXTURES / "display-v2"
    checkpoint_value = json.loads((root / "checkpoint.json").read_text())
    command_value = json.loads((root / "command-tick.json").read_text())
    assert len(validate_display_checkpoint(checkpoint_value)) == 2
    assert len(
        validate_display_command_stream(
            command_value,
...
def test_frozen_wire_v2_malformed_corpus_fails_closed() -> None:
    root = FIXTURES / "wire-v2"
    manifest = json.loads((root / "malformed-manifest.json").read_text())
    assert manifest["schema"] == "scene-engine-malformed-corpus@2"
    for name in manifest["files"]:
        with pytest.raises(WireError):
            read_engine_packet((root / name).read_bytes())


def test_wire_v2_commit_always_contains_a_command_stream_seal() -> None:
    ...
    assert packet.attachments[1].value["commands"][0]["name"] == "py/aircraft-17"
...
def test_validated_stream_fast_path_is_byte_exact_and_plain_data_stays_fail_closed() -> None:
    ...
    plain_stream = json.loads(json.dumps(stream))
...
def test_wire_v1_major_is_rejected_without_a_compatibility_decoder() -> None:
    display = encode_display_checkpoint(
        scene_name="main",
        catalog=DisplayCatalogIdentity("a" * 64, "b" * 64, "c" * 64),
        last_command_seq=0,
...
def test_old_raw_scene_attachment_layout_is_rejected() -> None:
    display = encode_display_checkpoint(
        scene_name="main",
        catalog=DisplayCatalogIdentity("a" * 64, "b" * 64, "c" * 64),
        last_command_seq=0,
...
def test_display_stream_cursor_and_tick_must_match_packet_header() -> None:
    ...
    for before, after in (
        (b'"last_command_seq":1', b'"last_command_seq":0'),
        (b'"base_command_seq":0', b'"base_command_seq":1'),
        (b'"source_tick":1', b'"source_tick":2'),
    ):
        malformed = raw.replace(before, after, 1)
