...
def _write_wheel(path: Path, files: dict[str, bytes]) -> None:
    with zipfile.ZipFile(path, mode="w", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr("scene_engine-0.7.0.dist-info/METADATA", b"ignored metadata\n")
...
def test_python_wheel_rejects_stale_package_files_but_ignores_dist_info(tmp_path: Path) -> None:
    ...
    source_root.mkdir()
    (source_root / "__init__.py").write_bytes(b'__version__ = "0.7.0"\n')
    (source_root / "display.py").write_bytes(b"DISPLAY_CODEC = 'scene-engine-display-node@2'\n")
    cache = source_root / "__pycache__"
    cache.mkdir()
    (cache / "scene.cpython-312.pyc").write_bytes(b"ignored interpreter cache")
    wheel = tmp_path / "scene_engine-0.7.0-py3-none-any.whl"
    _write_wheel(
        wheel,
        {
