...
It is separate from the Display authority root's column-major float32 Matrix4: rule space uses scaled decimal integers, while
Python keeps authority matrices in one little-endian float32 NumPy pool, transports checkpoint/dirty rows as contiguous
`(n,4,4)` / `(m,4,4)` tensors, and leaves matrix-semantic validation to Client.
