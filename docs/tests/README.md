...
| [`test_pointer_events.py`](../../tests/test_pointer_events.py) | 九种 authority Node 指针事件经现有 input 的编解码、同一 Node ID/name、Python 同步分发、作用域和监听移除。 |
...
| 测试项目 | 覆盖范围 |
| --- | --- |
| [`public.test.mjs`](../../js/packages/display/test/public.test.mjs) | Display 根包精确公开导出（含 pointer target/query/controller 与 Node event hub/input adapter）和内部 Authority mutation 类型隔离。 |
